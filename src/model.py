...
class TFiLMSuperResolution(nn.Module):
    ...
    def __init__(self, 
                 ...
                 max_filters=512,
                 quality_mode=False):  # If True, maintain input length and focus on quality
        ...
        # Add a list of BatchNorm1d layers in the constructor, and apply them before TFiLM
        self.batchnorm_layers = nn.ModuleList()
        
        # Define architecture arrays (mirroring TF version)
        n_filters = [128, 256, 512, 512, 512, 512, 512, 512]
        n_filter_sizes = [65, 33, 17, 9, 9, 9, 9, 9, 9]
        block_sizes = [128, 64, 32, 16, 8, 8, 8, 8]

        # Limit number of blocks based on your chosen depth
        n_layers = min(num_blocks, len(n_filters))
        
        # Downsampling path
        in_ch = input_channels
        for i in range(n_layers):
            out_ch = n_filters[i]
            kernel_size = n_filter_sizes[i]
            block_size = block_sizes[i]
            dilation = 2  # as in TF model
            ...
            # Normalization + modulation (TFiLM)
            self.batchnorm_layers.append(nn.BatchNorm1d(out_ch))
        ...
        for i in range(self.num_upsample_steps):
            skip_ch = n_filters[num_blocks - i - 1]  # channels from the skip connection
            in_ch_up = n_filters[num_blocks - i - 1]  # input to upsample (from bottleneck or prev up)
            out_ch_up = n_filters[num_blocks - i - 2] if (num_blocks - i - 2) >= 0 else base_channels
            kernel_size = max(128 // (2 ** (num_blocks - i - 1)) + 1, 9)

            self.up_blocks.append(
                UpsamplingBlock(in_ch_up + skip_ch, out_ch_up, kernel_size, scale_factor=2, dilation=1)
            )
            print(f"Up block {i}: in={in_ch_up + skip_ch}, out={out_ch_up}")
    ...
    def forward(self, x):
        ...
        # print(f"TFiLMSuperResolution forward(self,x) Input shape: {x.shape}")
        ...
        for i in range(self.num_blocks):
            ...
            # Apply BN before FiLM This replicates the paper’s normalization step and improves training stability
